...
function fixRange(stdClass &$current, stdClass &$cache): void
{
    ...
    if ($currentHasFrom === false && $cacheHasFrom === true) {
        ...
        ( $currentHasFrom === true && $cacheHasFrom === true )
        && ( $cache->from === $current->from )
    ) {
            ...
            if (isset($variables[$identifier]) === false) {
                ...
                foreach ((array) $doc as $key => $val) {
                    if (isset($variables[$identifier]->$key)) {
                        ...
                        $docValue   = $doc->$key;
                        if (
                            (                            strtoupper(json_encode($cacheValue)) === strtoupper(json_encode($docValue)))
                            ...
                            if ($key === 'type') {
                                ...
                                if (
                                    ...
                                    strtoupper(json_encode(ksort($docValue))) === strtoupper(json_encode(ksort($cacheValue)))
                                ) {// uppercase / lowercase
                                    ksort($cacheValue);
                                ...
                                if (
                                    str_replace($replaceSource, $replaceDest, $docValue) === str_replace($replaceSource, $replaceDest, $cacheValue)
                                ) {//Try replacements
                                    $newData->$key = str_replace($replaceSource, $replaceDest, $docValue);
                                    $nbrConflictsSolved++;
                                } elseif (
                                    str_replace('--', '', $docValue) === str_replace('--', '', $cacheValue)
                                    ...
                                    strlen(str_replace($cacheValue, '', $docValue)) !== strlen($docValue)
                                ) {// contained in cache
                                    $newData->$key = $docValue;
                                    $nbrConflictsSolved++;
                                } elseif (
                                    strlen(str_replace($docValue, '', $cacheValue)) !== strlen($cacheValue)
